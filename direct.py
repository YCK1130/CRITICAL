...
from src.prompts.direct import Direct
...
def check_security(commit_message:str, diff: list, with_example: bool = False, storepath: str = './data'):
    direct = Direct(commit_message=commit_message, diffs=diff, with_example=with_example)
    direct_comment, info = chat(item={"system": direct.system_setting, "prompts": [direct.prompts]})
    store_report(storepath, 'direct', direct_comment, info)
    print('Direct Done')
    print(f'Please check the report in {storepath}')
...
if __name__ == '__main__':
    ...
    for commit in tqdm.tqdm(commits):
        ...
        storepath = os.path.join('./data', filepath, branch,  f"direct_{target_commit[:6]}-{model}-{date}")
        ...
        check_security(commit_message, diff, with_example, storepath)
        print('-'*40)
